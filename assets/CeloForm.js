...
export default function CeloForm({ provider, account, connectWallet }) {
  ...
  const MAIN_WEBSITE_API = 'https://appsiko.com/api/get-token-data?network=celo';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.log(`Using user config: wallet=${userWallet}, fee=${userFee} CELO`);
      } catch (error) {
  ...
  const handleViewToken = () => {
    if (contractAddress) {
      const etherscanUrl = `https://celoscan.io/token/${contractAddress}`;
    ...
    <div className="container mx-auto p-6 max-w-3xl bg-background">
      ...
      <div className="mt-6 p-6 bg-muted rounded-xl animate-fadeIn">
        <h2 className="text-2xl font-bold font-sans text-black mb-4 relative">
          How to Create Your Token on the Celo Blockchain
        ...
        <p className="text-black font-bold font-sans leading-relaxed mb-4">
          Follow These Simple Steps To Create Meme Coin At Celo Within 10 - 30 Seconds:
        </p>
